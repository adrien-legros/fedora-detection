...
class ort_v5:
    ...
    def __call__(self, img_data, conf_thres, iou_thres):
        ...
        start = time.time()
        ort_sess = ort.InferenceSession('model/model.onnx')
        out = ort_sess.run(None, {'images': im})
        end = time.time()
        print(f"Inference took {end - start:.2f}ms")
        ...
        output = output.reshape(1, 25200, 6) # Reshape the flat array prediction
        out = self.non_max_suppression(output, conf_thres, iou_thres)[0] # Run NMS to remove overlapping boxes
        out_img = self.result(image_or, ratio, dwdh, out)
    ...
    def result(self,img,ratio, dwdh, out):
        names= self.class_name()
        colors = {name: [0,0,255] for i,name in enumerate(names)}
